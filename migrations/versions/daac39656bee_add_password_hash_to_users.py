...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('firstname', sa.String(length=100), nullable=False, server_default=''))
    op.add_column('users', sa.Column('lastname', sa.String(length=100), nullable=False, server_default=''))
...
def downgrade() -> None:
    """Downgrade schema."""
    op.add_column('users', sa.Column('name', sa.VARCHAR(length=100), autoincrement=False, nullable=False, server_default=''))
    op.alter_column('users', 'role',
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('user_infos_user_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('user_infos_pkey')),
    sa.UniqueConstraint('user_id', name=op.f('user_infos_user_id_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
