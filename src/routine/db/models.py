...
class RoutinesModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "name", name="uq_routine_user_id_name"
        ),  # Assure qu'un même utilisateur ne peut pas avoir deux routines avec le même nom
    )
    ...
    type = Column(
        Enum(RoutineType, name="routine_type"), nullable=False, server_default="TEXT"
    )
    multiselect_options = Column(
        JSON, nullable=True, server_default="[]"
    )  # option dans le cas d'une routine de type multiselect
