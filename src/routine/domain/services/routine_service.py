...
class RoutineService:
    ...
    def add_routine(
        ...
        if type == RoutineType.MULTISELECT.name and not multiselect_options:
            raise RoutineMissingOptionsError(
                "Les options sont obligatoires pour une routine de type Multiselect"
            )

        if self.repo.find_by_user_id_and_name(user_id, name):
            raise RoutineAlreadyExistsError(
                "Une routine existe déjà avec ce nom pour cet utilisateur"
            )

        # Suppression des '"' dans les options du multiselect pour éviter les problèmes de parsing JSON
        if multiselect_options:
            multiselect_options = [
                option.replace('"', " ") for option in multiselect_options
            ]
